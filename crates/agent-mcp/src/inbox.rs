...
//!
//! Until ADR-001 these lived in `agent-core` behind an `io` feature
//! and were written by the daemon, read by the MCP server. With the
//! daemon gone, only `agent-mcp` touches the inbox file, so the
//! helpers moved here.
//!
//! The inbox is a JSONL file (one [`InboxEntry`] per line). Brokers
//! (P4 / #16) call [`append`] when a `NotifyMessage` lands for this
//! agent; the `get_inbox` MCP tool calls [`read_all`] and then renames
//! the file aside so each line surfaces exactly once.
...
pub struct InboxEntry {
    ...
    /// Millis since epoch, stamped at the broker (or sender, for brokers
    /// without a server-side clock).
    pub timestamp: i64,
}
...
/// not exist. Malformed lines are skipped silently for now; structured
/// logging lands with #11.
pub async fn read_all(path: impl AsRef<Path>) -> Result<Vec<InboxEntry>> {
    ...
    while let Some(line) = lines.next_line().await? {
        ...
        if let Ok(entry) = serde_json::from_str::<InboxEntry>(&line) {
            entries.push(entry);
        }
    }
    Ok(entries)
